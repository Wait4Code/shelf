...
export type SeriesFieldProps = Readonly<{ name: string; visibleSubFields?: string[] }>;

export const SeriesField: React.FC<SeriesFieldProps> = ({name, visibleSubFields}) => {
    const {values, setFieldValue} = useFormikContext<Record<string, any>>();
    const current = (values as any)[name];

    // Initialiser Series avec des valeurs vides si c'est null
    React.useEffect(() => {
        if (current === null) {
    ...
    return (
        <Box>
            <Typography variant="h6">Série</Typography>
            <Box sx={{pl: 2}}>
                {show('title') && (<TextField fullWidth margin={"dense"} variant={"standard"} label={"Titre de la série"} error={titleMeta.touched && Boolean(titleMeta.error)} helperText={titleMeta.touched && titleMeta.error} {...titleField}/>)}
